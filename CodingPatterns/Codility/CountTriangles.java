...
public class CountTriangles {
    ...
    public int solution(int[] A) {
        ...
        // important: sort the edges so that we just need to check if "1st edge + 2nd edge > 3rd edge"
        Arrays.sort(A);
        
        // Using "Caterpillar method", so we can have O(n^2), not O(n^3)  
        for(int i=0; i < A.length-2; i++){
            ...
            while(leftEnd < A.length-1){
                ...
                if(rightEnd < A.length && A[i] + A[leftEnd] > A[rightEnd]){
                    ...
                    // note: need to minus "1" because the rightEnd is not included
